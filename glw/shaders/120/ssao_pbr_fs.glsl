...
{		
    ...
    if (u_EnableAlbedoTexture == 1 && render_mode == 3 && simpleRender == 0) {
        vec4 cv4 = texture2D(u_AlbedoTexture, v_Texcoord);
        albedo = cv4.rgb;
        alpha = cv4.a;
    }
    ...
    if (u_enableAO == 1) {
        ...
        ao = texture2D(u_AOMap, screenCoord).r;
    }
    ...
    if (simpleRender < 0.5){
        ...
        if (render_mode == 3 || render_mode == 1){
        ...
        // render mode normal
            ...
            // gl_FragColor = vec4((1.0-N)*0.4 + 0.2, 1.0);
